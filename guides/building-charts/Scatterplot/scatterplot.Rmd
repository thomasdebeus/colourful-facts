...
# Change Country Code column name in "Code"
infrastructure <- rename(infrastructure, Code = `Country Code`)
```
...
merged$quality.of.infastructure <- as.numeric(merged$quality.of.infastructure)
# Return rows with no NA data
merged <- merged[complete.cases(merged),]
# Change column names
merged <- merged %>% 
  rename(infrastructure = quality.of.infastructure,
         traffic.deaths = traffic.deaths.100000,
         alcohol.consumption = Total,
         Country = Country.x)
# Change country column name
merged <- merged %>% rename(Country = Country.x)
```

## Fianally time for plotting
```{r plot1-all-countries}
merged %>% 
